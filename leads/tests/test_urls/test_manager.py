...
@pytest.mark.usefixtures("test_manager", "test_support_team", "test_sales_team", "test_lead")
...
class TestLeadUrlsForAdmin:
    ...
    def test_lead_update_url(self, test_manager, test_lead):
        response = self.client.get(reverse("leads:lead-update", kwargs={"pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"

    def test_lead_detail_url(self, test_manager, test_lead):
        response = self.client.get(reverse("leads:lead-detail", kwargs={"pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"
    
    def test_lead_delete_url(self, test_manager, test_lead):
...
# /<slug:team_slug>/ -- not own team
    ...
    def test_lead_update_url_with_team_slug(self, test_manager, test_sales_team, test_lead):
        response = self.client.get(reverse("teams:leads:lead-update", kwargs={"team_slug": test_sales_team.slug, "pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"

    def test_lead_detail_url_with_team_slug(self, test_manager, test_sales_team, test_lead):
        response = self.client.get(reverse("teams:leads:lead-detail", kwargs={"team_slug": test_sales_team.slug, "pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"
    
    def test_lead_delete_url_with_team_slug(self, test_manager, test_sales_team, test_lead):
        response = self.client.get(reverse("teams:leads:lead-delete", kwargs={"team_slug": test_sales_team.slug, "pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"
...
# /leads/my-leads/
    ...
    def test_lead_update_url_my_leads(self, test_manager, test_lead):
        response = self.client.get(reverse("leads:my-lead-update", kwargs={"pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"

    def test_lead_detail_url_my_leads(self, test_manager, test_lead):
        response = self.client.get(reverse("leads:my-lead-detail", kwargs={"pk": test_lead.pk}))
        assert response.status_code == 302
        assert response.url == f"/accounts/dashboard/{test_manager.user.username}/"
    
    def test_lead_delete_url_my_leads(self, test_manager, test_lead):
